...
from utilities import create_item_widgets, resource_path, check_items, like_search
from available_search_result_page import AvailableResultFrame

class AvailableFrame(tk.Frame):   # 求物界面
    ...
    def create_wanting_labels(self):
        ...
        wanting_label = ttk.Label(wanting_title_frame, text='已发布的闲置信息', style='ThemeFont.TLabel')
        wanting_label.pack(side='left')

        search_entry = ttk.Entry(wanting_title_frame, width=30, font=("Arial 16"), textvariable=self.keyword, takefocus=False)
        search_entry.pack(side='left', padx=(20, 10))

        search_button = ttk.Button(wanting_title_frame, text="搜索", style='ButtonFont.TButton', command=self.main_page_search)
        ...
        create_item_widgets(wanting_item_frames, row_number, "闲置中", self.frames)

    def main_page_search(self):
        keyword = self.keyword.get()
        item_list, row_number = like_search(keyword)

        self.search_result_frame=AvailableResultFrame(self.root, self.frames, keyword, item_list, row_number)
        self.frames["search_result"] = self.search_result_frame
        self.pack_forget()
